...
player_pos = [10.0, 0.0, 10.0] # initial player position
player_yaw = 0.0  # initial player yaw
player_pitch = 0.0 # initial player pitch
cam_speed = 0.5
yaw_speed = 2.0
room_size = 20.0
wall_tiles = []
...
def draw_gun_fps():
    glPushMatrix()
    glTranslatef(0.2, -0.3, -0.8) # position of the gun
    glPushMatrix()
    glColor3f(0.15, 0.15, 0.15)
    gluCylinder(gluNewQuadric(), 0.07, 0.07, 0.2, 32, 8) # gun body
    glPopMatrix()
    glPushMatrix()
    glColor3f(0.1, 0.1, 0.1)
    glTranslatef(0.0, 0.0, 0.2) # position of the barrel
    gluCylinder(gluNewQuadric(), 0.05, 0.05, 0.6, 32, 8) # gun barrel
    glPopMatrix()
    glPushMatrix()
    glColor3f(0.12, 0.12, 0.12)
    glTranslatef(0.0, -0.15, 0.05)# position of the trigger
    glRotatef(75, 1, 0, 0)
    glScalef(0.08, 0.3, 0.1) # trigger shape
    glutSolidCube(0.5) # trigger cube
    glPopMatrix()
    glPopMatrix()

def draw_bullet(bullet):
    glPushMatrix()
    glTranslatef(bullet.pos[0], bullet.pos[1], bullet.pos[2]) # position of the bullet
    if bullet.color == 'blue':
        glColor3f(0.0, 0.0, 1.0)
    else:
        glColor3f(1.0, 1.0, 0.0)
    glutSolidSphere(BULLET_RADIUS, 16, 16) # bullet shape
    glPopMatrix()
...
def draw_crosshair():
    ...
    glPushMatrix()
    glLoadIdentity()
    glOrtho(0, 1, 0, 1, -1, 1) # set orthographic projection
    glMatrixMode(GL_MODELVIEW) 
    glPushMatrix()
    glLoadIdentity()
    glDisable(GL_DEPTH_TEST) # disable depth test
    glColor3f(0, 0, 0)
    cx, cy = 0.5, 0.5 # center of the screen
    r_outer = 0.015 # outer circle radius
    r_dot = 0.002 # inner circle radius
    glBegin(GL_LINE_LOOP) # draw outer circle
    for i in range(64):
        angle = 2.0 * math.pi * i / 64
        glVertex2f(cx + math.cos(angle) * r_outer, cy + math.sin(angle) * r_outer)
    glEnd()
    glBegin(GL_POLYGON) # draw inner circle
    for i in range(32):
        angle = 2.0 * math.pi * i / 32 # angle
        glVertex2f(cx + math.cos(angle) * r_dot, cy + math.sin(angle) * r_dot) # vertex
    glEnd()
...
def ray_aabb_intersection(start, end, aabb_min, aabb_max): # AABB intersection test
    tmin = 0.0
    tmax = 1.0
...
def update_player_physics(dt): # player physics update
...
def timer(value): # timer function
    update_bullets(0.016)  # update bullets
    update_player_physics(0.016) # update player physics
    glutPostRedisplay()
    glutTimerFunc(16, timer, 0) # 16 ms delay
...
def mouse_motion(x, y): # mouse motion function
    global player_yaw, player_pitch, mouse_captured # mouse capture
    if game_won: 
        return
    if not mouse_captured:
        return
